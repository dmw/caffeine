...
caf_ssm_t *
caf_ssm_new (int id, int cycle, int steps) {
	caf_ssm_t *r = (caf_ssm_t *)NULL;
	int c;
	if (id > 0 && steps > 0) {
...
int
caf_ssm_delete (caf_ssm_t *r) {
	int c;
	if (r != (caf_ssm_t *)NULL) {
		if (r->m_state != (caf_ssm_state_t **)NULL) {
			for (c = 0; c < r->steps; c++) {
				if (r->m_state[c] != (caf_ssm_state_t *)NULL) {
					xfree (r->m_state[c]);
				}
			}
			xfree (r->m_state);
...
caf_ssm_state_t *
caf_ssm_state_new (int s_id, caf_ssm_state_type_t s_type, CAD_SSM_CALL(s_call),
                   CAD_SSM_CALL(s_error)) {
	caf_ssm_state_t *r = (caf_ssm_state_t *)NULL;
	if (s_id > 0 && s_call != (void *)NULL) {
		r = (caf_ssm_state_t *)xmalloc (CAF_SSM_STATE_SZ);
		if (r != (caf_ssm_state_t *)NULL) {
			r->s_id = s_id;
			r->s_type = s_type;
			r->s_call = s_call;
			r->s_error = s_error;
...
int
caf_ssm_state_delete (caf_ssm_state_t *r) {
	if (r != (caf_ssm_state_t *)NULL) {
		xfree (r);
		return CAF_OK;
	}
...
int
caf_ssm_add (caf_ssm_t *m, caf_ssm_state_t *s) {
	if (s != (caf_ssm_state_t *)NULL && m != (caf_ssm_t *)NULL) {
		m->last++;
		m->m_state[m->last] = s;
		return CAF_OK;
	}
...
int
caf_ssm_remove (caf_ssm_t *m, caf_ssm_state_t *s) {
	int c;
	if (s != (caf_ssm_state_t *)NULL && m != (caf_ssm_t *)NULL) {
		for (c = 0; c <= m->last; c++) {
			if (m->m_state[c] == s) {
				m->m_state[c] = (caf_ssm_state_t *)NULL;
				return CAF_OK;
			}
		}
	}
...
caf_ssm_runner_t *
caf_ssm_runner_new (caf_ssm_t *m, int r_id, caf_ssm_state_control_t ctrl) {
	caf_ssm_runner_t *r = (caf_ssm_runner_t *)NULL;
	if (m != (caf_ssm_t *)NULL && r_id > 0) {
		r = (caf_ssm_runner_t *)xmalloc (CAF_SSM_RUNNER_SZ);
		if (r != (caf_ssm_runner_t *)NULL) {
			r->r_id = r_id;
			r->r_machine = m;
			r->r_control = ctrl;
			r->r_state = (caf_ssm_state_t *)NULL;
			r->r_call = (void *)NULL;
			r->r_data = (void *)NULL;
...
int
caf_ssm_runner_delete (caf_ssm_runner_t *r) {
	if (r != (caf_ssm_runner_t *)NULL) {
		xfree (r);
		return CAF_OK;
	}
